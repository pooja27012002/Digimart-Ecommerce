...
export const CartComp = () => {
  ...
  const handleQuantityChange = async (cartItemId, productId, change) => {
    try {
      const response = await fetch(`http://localhost:8080/cart/updateItems/cartitemid/${cartItemId}/productid/${productId}/quantity/${change}`, { method: 'PUT' });
      if (!response.ok) throw new Error('Network response was not ok');
  ...
  const handleRemoveFromCart = async (cartItemId, productId) => {
    try {
      const response = await fetch(`http://localhost:8080/cart/cartId/${userId}/productId/${productId}`, { method: 'DELETE' });
      if (!response.ok) throw new Error('Network response was not ok');
      await fetchCartItems(); // Refresh cart items
      await fetchCart(); // Refresh cart
    } catch (error) {
  ...
  return (
    <main className="py-12 max-w-7xl container mx-auto px-4">
      <div className="container mx-auto">
        <h2 className="mb-5 ml-7 mt-11 text-3xl text-yellow-600 font-bold">Shopping Cart</h2>
